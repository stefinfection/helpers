...
for cancerType in cancerTypes:
	print('Requesting ' + cancerType + '...') 
	fmtdType = cancerType.replace('_', ' ')
	cancerData[fmtdType] = []
	...
	if len(pqList) == 0:
		print('Warning, ' + cancerType + ' returned an empty list')
	
	for pq in pqList:
		gene = pq.find("i")
		cancerData[fmtdType].append(gene.string)

with open('genes_by_cancer_type_ncgv6.json', 'w') as outfile:
	json.dump(cancerData, outfile)

print('completed creating file for all cancer Types')
