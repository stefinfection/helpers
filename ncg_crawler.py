...
# arg parse and check
if (len(sys.argv) < 2):
	print("please enter a mode: (cancer or tissue)")
	sys.exit()

mode = sys.argv[1]
if (mode != CANCER and mode != TISSUE):
	print("please enter a valid mode: must be cancer or tissue")
	sys.exit()

print('Scraping for ' + mode + ' types...')
...
for type in types:
	...
	# capitalize each word in type + add to dict
	fmtdArr = type.replace('_', ' ').split()
	fmtdType = ''
	for w in fmtdArr:
		fmtdType += (w.capitalize() + ' ')
	fmtdType = fmtdType[:-1]
	data[fmtdType] = []

	if (mode == CANCER):	
		ctObj = { 'cancerGenesWhat': 'OR', 'primarySitesWhat': 'OR', 'cancerTypesWhat': 'OR', 'cancerTypes[]': type, 'FunctionalClassWhat': 'OR', 'Duplicability': 'no_filter', 'Connectivity': 'no_filter', 'miRNA': 'no_filter', 'essentiality': 'no_filter', 'advanced_search': 'advanced_search', 'Submit': 'Get list!' }
	else:
		ctObj = { 'cancerGenesWhat': 'OR', 'primarySitesWhat': 'OR', 'cancerTypesWhat': 'OR', 'primarySites[]': type, 'FunctionalClassWhat': 'OR', 'Duplicability': 'no_filter', 'Connectivity': 'no_filter', 'miRNA': 'no_filter', 'essentiality': 'no_filter', 'advanced_search': 'advanced_search', 'Submit': 'Get list!' }
...
with open('gene_list_by' + mode + '_type_ncgv6.json', 'w') as outfile:
